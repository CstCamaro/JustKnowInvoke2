...
Province_City[ArrahLength][2][10]=Array("乌兰浩特市","阿尔山市","突泉县","扎赉特旗","科尔沁右翼前旗","科尔沁右翼中旗");
Province_City[ArrahLength][2][11]=Array("阿拉善左旗","阿拉善右旗","额济纳旗");
Province_City[ArrahLength][3] = 1;
ArrahLength++;
...
Province_City[ArrahLength][1]=new Array("哈尔滨市","齐齐哈尔市","鸡西市","鹤岗市","双鸭山市","大庆市","伊春市","牡丹江市","佳木斯市","七台河市","黑河市","绥化市","大兴安岭地区");
Province_City[ArrahLength][2] = new Array();
...
Province_City[ArrahLength][2][4]=Array("金阊区","沧浪区","平江区","虎丘区","吴中区","相城区","常熟市","张家港市","太仓市","昆山市","吴江市","工业园区");
...
{	
	...
	//$("#region").setinfo({empty:false,inputtype:"",initmsg:"",msg_id:"province_msg",errmsg:"",sussmsg:""});
	if(InitName_2[listid]!='') $("#"+InitName_2[listid]).setinfo({empty:false,inputtype:"",initmsg:"",msg_id:InitName_0[listid]+"_msg",errmsg:"",sussmsg:""});
	...
	if(ishaveregion){
		if(Province_City[value][3]==0){
			$('#'+InitName_1[listid]).val('1');
			$('#'+InitName_1[listid]).hide();
			Changeregion(value,0,InitName_2[listid]);	

			$('#'+InitName_1[listid]).val(1);
			$('#'+InitName_2[listid]).val(0);			
			if(typeof(Province_City[value][2])!="undefined" && typeof(Province_City[value][2][0])!="undefined" && Province_City[value][2][0].length >0){
				//alert('1');
				$("#"+InitName_2[listid]+"_select_span").show();
				$('#'+InitName_1[listid]).setinfo({empty:true,inputtype:"",initmsg:"",msg_id:InitName_0[listid]+"_msg",errmsg:"必选项",sussmsg:""});
				$('#'+InitName_2[listid]).setinfo({empty:true,inputtype:"",initmsg:"",msg_id:InitName_0[listid]+"_msg",errmsg:"必选项",sussmsg:""});
			}else{
				//alert('2');
				$("#"+InitName_2[listid]+"_select_span").hide();
				$('#'+InitName_2[listid]).setinfo({empty:false,inputtype:"",initmsg:"",msg_id:InitName_0[listid]+"_msg",errmsg:"",sussmsg:""});
				$('#'+InitName_1[listid]).setinfo({empty:true,inputtype:"",initmsg:"",msg_id:InitName_0[listid]+"_msg",errmsg:"必选项",sussmsg:""});
			}
		}else{
			$('#'+InitName_1[listid]).fadeIn("slow");
			$("#"+InitName_2[listid]+"_select_span").hide();
			//Changeregion(value,0,InitName_2);	
			$('#'+InitName_1[listid]).val(0);
			$('#'+InitName_2[listid]).val(0);			
			if(typeof(Province_City[value][2])!="undefined" && typeof(Province_City[value][2][0])!="undefined" && Province_City[value][2][0].length >0){
				//alert('3');
				$('#'+InitName_1[listid]).setinfo({empty:true,inputtype:"",initmsg:"",msg_id:InitName_0[listid]+"_msg",errmsg:"必选项",sussmsg:""});
...
function Changeregion(value,citypos,obSelectCity){
	...
	if(value<0)	{
		$("#"+obSelectCity+"_select_span").hide();
		return false;
	}
	...
	AddOption ("请选择区域", "0", obSelectCity);
	var ishide = true;
	...
	if(!ishide){
		$("#"+obSelectCity+"_select_span").show();
		$('#'+InitName_1[listid]).setinfo({empty:true,inputtype:"",initmsg:"",msg_id:InitName_0[listid]+"_msg",errmsg:"必选项",sussmsg:""});
		$('#'+InitName_2[listid]).setinfo({empty:true,inputtype:"",initmsg:"",msg_id:InitName_0[listid]+"_msg",errmsg:"必选项",sussmsg:""});
	}else{
		$("#"+obSelectCity+"_select_span").hide();
...
function Writepageinit(ObjectElementname,ProvinceValue,CityValue,RegionValue,SArray,returnflag){
	...
	if(Province =='全国') {
		if(returnflag==0) $("#"+ObjectElementname).html('全国');
		else return '全国';
	}
	...
	if(SArray[ProvinceValue-1][3]==1){
		...
		//$("#"+ObjectElementname).html(Province+"省"+City+"市"+Region);
		if(returnflag==0) $("#"+ObjectElementname).html(Province+"省"+City+Region);
		...
		//$("#"+ObjectElementname).html(Province+"市"+Region);
		if(returnflag==0) $("#"+ObjectElementname).html(Province+Region);
		else return Province+"省"+City+Region;
	}
}
...
function ShowCityRegion(ObjectElementname,ProvinceValue,CityValue,RegionValue,SArray,returnflag){
	...
	if(SArray[ProvinceValue-1][3]==1){
		...
		if(returnflag==0){
			//if(ObjectElementname !='' &&  ObjectElementname!="-1") $("#"+ObjectElementname).html(City+"市"+Region);
			if(ObjectElementname !='' &&  ObjectElementname!="-1") $("#"+ObjectElementname).html(City+Region);
			...
			if(ObjectElementname !='' &&  ObjectElementname!="-1") $("#"+ObjectElementname).html(City);
		...
		if(returnflag==0){
			//if(ObjectElementname !='' &&  ObjectElementname!="-1") $("#"+ObjectElementname).html(Province+"市"+Region);
			if(ObjectElementname !='' &&  ObjectElementname!="-1") $("#"+ObjectElementname).html(Province+Region);
			...
			if(ObjectElementname !='' &&  ObjectElementname!="-1") $("#"+ObjectElementname).html(Province);
...
/*  |xGv00|90e2ff1d76fc24b272d9585ef7136ee8 */