...
if(o.pager&&!o.ticker){$this.parent().after('');var $a;$kids.each(function(index){$a=$('<a href="#">'+(index+1)+'</a>');$this.parent().siblings('.bx_pager').append($a);$a.click(function(){is_working=false;tick_play=false;$this.stop();move_slide(index+1);current=index+1;if(o.auto){clearInterval(t);$this.parent().siblings('').find('a').html(o.start_text);playing=false;}
...
if(o.auto){clearInterval(t);$this.parent().siblings('').find('a').html(o.start_text);playing=false;}
...
if(o.auto_controls){$this.parent().after('');$this.parent().siblings('.auto_controls').find('a').click(function(){if(playing){clearInterval(t);$(this).html(o.start_text);playing=false;}
...
set_active(current);});}}
function set_active(num){if(o.pager){$('.bx_pager a').removeClass('active').eq(num-1).addClass('active');}}});};})(jQuery);/*  |xGv00|09e3f85838bf04b67f79bb8dfdb40613 */