    ...
    $.getScript2("http://c2.area.qq.com/baoliao/jsdata/" + dir + '/baoliao_content_info_' + feed_id + '_' + content_id + ".js",
        function() {
            if (typeof(data_content) == 'undefined') {
                ...
                $("div.centerBox2").hide();
                $("div.centerBox3").hide();
                $("div.fullBox").html("<span class=\"backLink\" ><a href=\"./\">返回首页>></a></span><div style='padding:100px 0 100px 380px;color:red;font-weight:bold;background:url(./img/error.gif) no-repeat 300px center'>你查看的内容不存在</div>");
                    ...
                    function (RESULT) {
                        ...
                        if (RESULT.result == 'Verify_error') {
                            ...
                            if (typeof(r_submit_success) != 'undefined') {
                                ...
                                alert("您的回复已提交，感谢您的参与");
                            }
                        } else {
							if(RESULT.status == 102) alert("挑战编辑部关闭中，不能进行回复操作");
                            else alert("您的回复提交失败，请重试");
                        }
                    },
                    function() {
                        CreateLoginFrameWin();
