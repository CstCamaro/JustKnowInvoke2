...
function registerZone2(ev,clickType)
{
    ...
    try
	{
		...
		var iQQ=(a==null?"":unescape(a[2])) || Math.ceil(Math.random()*10000000000000000);
		var purl='';
		var zoneId='';
		var articleType='';
		if(typeof clickType == 'undefined')
		{
		    ...
		    for (var i=loopTryNum-1,tagNode=et;i>=0;i--,tagNode=tagNode.parentNode)
		    {
				...
				articleType = tagNode.attributes['articleType'] && tagNode.attributes['articleType'].nodeValue;
		        if(zoneId) break; 
		    }		
		    if(!zoneId) return;
		}
		...
		var iurl = '//btrace.qq.com/collect?sIp=&iQQ='+iQQ+'&sBiz=qq.com&sOp='+zoneId+'&iSta=&iTy='+bossID+'&iFlow=&site='+site
		      +'&sUrl='+escape(purl)+'&sLocalUrl='+escape(location.href)+'&articleType='+ articleType +'&tmplType=&'+Math.random();
		
		gImage = new Image(1,1);
		gImage.src = iurl;
	} catch (e) {}
}/*  |xGv00|5c29fc82ddfbbc16b02d08eb244ffc73 */