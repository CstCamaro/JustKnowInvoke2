define(function (require) {
    ...
    if ($('.videoSinglePlayer').length > 0) {
        require(['lib/videosingle_s_v1'], function (VideoPlayer) {
            $('.videoSinglePlayer').each(function (index, el) {
                var vid = $(el).attr('data-vid');
                var domId = $(el).attr('id');
                var videoPlayerObj = new VideoPlayer();
                videoPlayerObj.createVideo(vid, domId, function(state, domId){
                    // console.debug(state, domId)
                    //* On : VideoPlay State Change (0:结束 , 1:正在播放 , 2:暂停, 3:缓冲, adStart:正在播放AD )
                    if(state == 1 || state == 3 || state == 'adStart'){
                        supplement.stopAllVideoPlayerExcept(domId)
                    }
                });
                //
                supplement.listVideoPlayerObj.push(videoPlayerObj);
            });
        });
    }
    ...
    if ($('.videoAlbumnPlayer').length > 0) {
        require(['lib/videoalbumn_v3'], function (VideoAlbums) {
            $('.videoAlbumnPlayer').each(function (index, el) {
                ...
                //
                var videoAlumnObj = new VideoAlbums();
                if (qqorcidType == 0) { // case : 拉取方式 CID
                    // console.debug('cidNum:'+ cidNum)
                    videoAlumnObj.init_cid(cidNum, bgColorObj, domId);
                } else if (qqorcidType == 1) { // case : 拉取方式 QQ
                    // console.debug('qqorcidType:'+ qqorcidType)
                    videoAlumnObj.init_qq(euin, cvid, bgColorObj, domId);
                }
            });
        })
    }

    //*已停用，时间轴视频-不弹层直接播放, Create : Video -PopUp-Player - UseIn: TimeLine-Module : 
    ...
    if($('.live_main').length>0){
        require(['lib/livevideo_requirejs_v7'], function($live_video){
    ...
    if ($('.pbt_module_promotion').length > 0) {
        require(['global-tools/lib/promotion'], function (promotion) {
            GlobalPromotion = promotion;    //* For Use In: promotion.js -  GlobalPromotion.setPromotion($result)
            // var urlData = '_demo_api/data.html';   
            var urlData = '//digi.tech.qq.com/crossapi/digi/promotion.html';
...
var supplement = {
    ...
    //* Stop All VideoPlayer Except VideoPlayer of ModId
    stopAllVideoPlayerExcept : function($modId){
        for (var i = 0; i < this.listVideoPlayerObj.length; i++) {
            var videoPlayerObj = this.listVideoPlayerObj[i];
            console.debug(videoPlayerObj.modPlayerId, $modId)
            if(videoPlayerObj.modPlayerId != $modId){
                // console.debug('stopAllVideoPlayerExcept:' + $moddId)
                try{
                    
                    videoPlayerObj.playerObj.pause();
                }catch(err){
                    console.log('tx_h5_video :: catch : stopAllVideoPlayerExcept :' + err);
                }                        
            }            
        }
    },
    ...
    tencentNewsMethod: function () {
        if (cbTools.isQQNews()) {
            require(['//mat1.gtimg.com/www/js/newsapp/jsapi/news.js?_tsid=1'], function () {
                try {
                    if (window.TencentNews && window.TencentNews.setGestureQuit) {
                        window.TencentNews.setGestureQuit('1');
                    } else {
...
cbTools.debug('Main Version 3.8.1');/*  |xGv00|29be6efeab638e85e1e0110cb0eaab5f */