...
MI.FollowAll.prototype.follow = function(b, a, c) {
    ...
    if (c && d.followEl) MI.Login.showPopup(d.name, d.followEl);
        ...
        if (a.length) if (b.mbid) {
            mb_cbFollow.mbid = b.mbid;
			g_mi_followtype = 1;
            UI.getScript("http://mini.t.qq.com/follow.php?c=1&u=" + a.join(",") + "&R=" + (new Date).getTime(), null, "utf-8")
...
MI.FollowOne = function(o) {
	...
	Self.listEl = o.listId && UI.G(o.listId);
	Self.pref = o.pref;
...
MI.FollowOne.prototype.follow = function(infoObj, black, nologin) {
	...
	if (infoObj.mbid) {
		mb_cbFollow.mbid = infoObj.mbid;
		g_follow_pref = Self.pref;
		g_mi_followtype = 0;
		UI.getScript('http://mini.t.qq.com/follow.php?c=1&u='
			+ ids.join(',') + '&R=' + new Date().getTime(), null, 'utf-8');
	} else {
		window.open(idx);
	}
	Self.hide();
}
function mb_cbFollow(b) {
    if (b.result == -1) MI.Login.logout();	
    else if (b.result != 0) {
		alert("收听失败！");
	}
	else{
		if(1==g_mi_followtype)	MI.List.init();
	}
}
mb_cbFollow.mbid = "";
...
}/*  |xGv00|18e5768f65de78c4e634363a6dcbd689 */