define('project/login/oneLogin_', function(require, exports, module){
	//同域
	try {
	...
	window["login"] = {
		...
		isLogin: function(){
			return !!ui.cookie('skey') && !!ui.cookie('uin');
		},
		clsFun: function(){
		...
		loginSuccess:function(){
			...
			if(login.cbArr.length > 0){
				for(var i = login.cbArr.length-1; i >= 0; i--){
					setTimeout(function(){login.cbArr[i]();}, 0);
				}
			}
			//点击回调
			login.loginCheck();
	...
	//login E
});/*  |xGv00|cb24ae87040ac1660fcd55e188e46a9a */