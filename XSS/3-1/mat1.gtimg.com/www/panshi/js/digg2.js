...
if (typeof AppPlatform.Survey.Digg == "undefined") {
  AppPlatform.Survey.Digg = {
      ...
      loadDiggResult: function(projId) {
          ...
          var LoadUrl = "//panshi.qq.com/vote/" + AppPlatform.Survey.Digg.ProjectList[projId].ProjectId + "/result?source=1&callback=AppPlatform.Survey.Digg.showDiggResult&_=" + Math.random();
      ...
      digg: function(obj, projId, subjId, optId) {
          var qq_client_id = AppPlatform.Cookie.getCookie("qq_client_id");
          var qq_openid = AppPlatform.Cookie.getCookie("qq_openid");
          var qq_access_token = AppPlatform.Cookie.getCookie("qq_access_token");
          // 腾讯视频
          var vqq_appid = AppPlatform.Cookie.getCookie("vqq_appid");
          var vqq_openid = AppPlatform.Cookie.getCookie("vqq_openid");
          var vqq_access_token = AppPlatform.Cookie.getCookie("vqq_access_token");
          // 微信登录
          var wx_head = AppPlatform.Cookie.getCookie("wx_head");
          var appid = AppPlatform.Cookie.getCookie("appid");
          var access_token = AppPlatform.Cookie.getCookie("access_token");
          var openid = AppPlatform.Cookie.getCookie("openid");
          var qq_navWrap = document.getElementById("qq_navWrap");
          if (qq_navWrap) {
              if (!(qq_client_id && qq_openid && qq_access_token)
                  && !(vqq_appid && vqq_openid && vqq_access_token)
                  && !(wx_head && appid && access_token && openid)) {
                  alert("\u8bf7\u5148\u767b\u5f55");
                  return;
              }
          }
          ...
          */
          ...
          if (AppPlatform.Survey.Digg.ProjectList[projId].DiggMode !== 2) {
            AppPlatform.$('apps_svy_opt_title_' + projId + '_' + IndexId).innerHTML =
          ...
          if (qq_client_id && qq_openid && qq_access_token) {
            data.g_tk = generateToken(qq_access_token);
            data.qq_client_id = qq_client_id;
            data.qq_openid = qq_openid;
            data.qq_access_token = qq_access_token;
          } else if (vqq_appid && vqq_openid && vqq_access_token) {
            // 腾讯视频
            data.g_tk = generateToken(vqq_access_token);
            data.qq_client_id = vqq_appid;
            data.qq_openid = vqq_openid;
            data.qq_access_token = vqq_access_token;
          } else if (wx_head && appid && access_token && openid) {
            // 微信登录
            data.g_tk = generateToken(access_token);
            data.login = 2;
            data.appid = appid;
            data.access_token = access_token;
            data.openid = openid;
          } else {
            data.g_tk = generateToken(getKey());
          }
...
}/*  |xGv00|3f62c1325ffbf67e62cb5897ffa6fbdc */