...
citys.getUrl = function(key,area,id){
	var add ={
        'hq':'http://' + area +'.auto.qq.com/15index/15foc/hangqing.htm',
        'jj':'http://wecar.qq.com/api/lowprice/hotLowPriceSerial?cityid=' + id + '&pagesize=5&format=jsonp',
        'hqall':'http://auto.qq.com/15index/15foc/hangqing.htm',
        'all':'http://wecar.qq.com/api/AutoHomePage/getInfos?city_id=' + id + '&format=jsonp'
    };
    return add[key];
};
//获取行情数据
	  	...
	  	success:function(data){
			...
			for(var i = 0; i < _data.length; i++){
				...
				seconds = Math.floor(d.getTime()/1000);
	  			...
	  			tpl += '<tr><td class="tl"><a href="http://auto.qq.com/jiangjia.htm?brandid=' + _data[i].sbrand_id + '&serialid=' + _data[i].sserial_id + '&cityid=' + _data[i].scity_id + '&pgv_ref=article';
...
citys.getbjpage = function(cityId) {
	var _url = 'http://wecar.qq.com/api/recomm/index?num=7&site=dwhome&provinceid=' + cityId + '?callback=call';
...
citys.getIpInfo =  function(_url){
	var hangqing = '',hangqingTemp='',shangjia = '',shangjiaTemp = '',chengxin = '',chengxinTemp1 = '',chengxinTemp2 = '',tips = '',tipsHead = '',tipsTemp1 = '',tipsTemp2 = '',tipsTemp3 = '',areaUrl = "http://beijing.auto.qq.com/";
	$.ajax({
		type: 'GET',
		url:_url,
		dataType:'jsonp',
...
/*
 ...
 citys.getESCInfo = function(cityId){
	$.ajax({
	  url:"http://ucar.qq.com/js/recommendlist?cityid=" + cityId + "&format=jsonp&callback=",
	  dataType: "jsonp",
	  callback: 'callback',
	  success:function(data){
...
citys.getEscMore = function(cityId){
	//http://ucar.qq.com/js/recommendlist?cityid=54&format=jsonp&callback=&callback=jQuery190022042583131765658_1512030253007&_=1512030253008(对接edyang)
	$.ajax({
	  url : 'http://ucar.qq.com/js/recommendlist',
	  data : {
		cityid : cityId,
		format : 'jsonp'
	  },
...
/*  |xGv00|fa9e93659bfcd0e5c6e7a9b7dc1ec747 */