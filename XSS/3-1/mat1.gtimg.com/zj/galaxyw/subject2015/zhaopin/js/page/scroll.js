		...
		duma.BeautifyScrollBar.prototype = { 
			...
			areaScroll:function() { 
				if( this.handle.style.display != "none" ){
					var topTemp = this.area.scrollTop/(this.area.scrollHeight - this.area.offsetHeight) * (this.bar.offsetHeight - this.handleHeight - this.arrowUpHeight - this.arrowDownHeight) + this.arrowUpHeight;
					isNaN( topTemp )?"":this.handle.style.top = topTemp+ "px" ;// 处理ie7 启用脚本调试后，NaNpx中断了脚本执行；
				}	
			}, 
...
});/*  |xGv00|d087468b77ac91733a47ad2114abe4b2 */