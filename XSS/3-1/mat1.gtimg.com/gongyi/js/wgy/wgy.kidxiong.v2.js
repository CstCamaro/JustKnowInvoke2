...
function orderWeiboTopic() {
	if(gIsLogin==1)
	{
		var url="http://wgyapp.gongyi.qq.com/wgy/orderWeiboTopic?" + Math.random() + "&topicid=" + weiboTopic[orderWeiboTopicID].id;
		$.getScript(url,
		function() {
...
function broadcastWeibo(flag){
	if(gIsLogin==1)
	{
		...
		var n=content.indexOf("了解更多：http://gongyi.qq.com/wgy");
		if(n<0)
			content+="了解更多：http://gongyi.qq.com/wgy";
		var url='http://wgyapp.gongyi.qq.com/wgy/broadcastWeibo?desc='+encodeURIComponent(content)+'&r='+Math.random();
		$.getScript(url,
		function(){
...
function showApplyDreamLayer() {
	...
	$.getScript('http://wgyapp.gongyi.qq.com/weimx/check/'+Math.random(),function(){
...
function doDonate() {
	...
	var H = "http://wgyapp.gongyi.qq.com/donate/add?" + Math.random() + "&uin=" + gOrganizer + "&dreamid=" + gDreamID + "&donateType=" + F + "&donateMoney=" + J + "&donateDesc=" + I;
	$.getScript(H,
	function() {
		if (typeof(rsObj) != "undefined" && rsObj != null) {
			if (rsObj.code == 0 || rsObj.code == 1) {
				...
				{
					var url='http://wgyapp.gongyi.qq.com/wgy/broadcastWeibo?desc='+I+'&r='+Math.random();
...
function showDreamList(page) {
	if (typeof(smalldreamlist) == "undefined") {
		...
		if(INIT==true){
			INIT=false;
			$.getScript("http://wgyapp.gongyi.qq.com/wgy/pageLoaded?s=10&r=" + Math.random(),function(){});
		}
...
function obtainWjj(flag) {
	hideTipLayer();
	hideTip3Layer();
	$.getScript("http://wgyapp.gongyi.qq.com/income/get?" + Math.random(),
	function() {
		if (typeof(rsObj) != "undefined" && rsObj != null) {
			if (rsObj.code == 0) {
				flushAccount();
...
function obtainWjj2() {
	hideTipLayer();
	hideTip3Layer();
	$.getScript("http://wgyapp.gongyi.qq.com/income/get2?" + Math.random(),
	function() {
		if (typeof(rsObj) != "undefined" && rsObj!= null) {
			if(rsObj.code==-2){
...
function listen() {
	...
	if (A.length < 1) {
		...
		$.getScript("http://wgyapp.gongyi.qq.com/wgy/getinfo?" + Math.random(),
		function() {
			if (typeof(rsObj) != "undefined" && rsObj != null) {
				if (rsObj.code == 0) {
					var B = "u=";
					B += A.join(",");
					var D = "http://radio.t.qq.com/mini/follow.php?" + B;
					D += "&callback=listenWeiboCallBack&uin=" + rsObj.uin + "&clientuin=&clientkey=&r=1311063176566";
					$.getScript(D)
				} else if (rsObj.code == -1){
					ptloginopenfun()
				}
...
function changeWeiboTopic(F) {
	...
	$("#weibotextarea").val(B + weiboTopic[F].desc+"了解更多：http://gongyi.qq.com/wgy");
...
function initIframeAccount(){
	$.getScript("http://wgyapp.gongyi.qq.com/account/get?" + Math.random(),
	function() {
...
function initAccount() {
	$.getScript("http://wgyapp.gongyi.qq.com/account/get?do=init&r=" + Math.random(),
	function() {
		if (typeof(accountObj) != "undefined" && accountObj != null) {
			if (accountObj.code == 1) {
				var nickname=fliterText(accountObj.info.nickname);
				$("#LoveplanLoingHeardObjectname").html('<span style="float:left">欢迎您，'+nickname + "&nbsp;&nbsp;成长阶段：</span><span class='gongyi_level "+(accountObj.info.lovestep>=1?'open':'close')+"'><i class='lv'></i><em class='num"+(String(accountObj.info.lovestep))+"'></em></span>&nbsp;&nbsp;<a href='http://gongyi.qq.com/mygongyi.htm' hidefocus>进入我的个人中心</a>&nbsp;&nbsp;<a href='javascript:ptlogoutopenfun();' hidefocus>[退出]</a></span>");
				var str='<div class="login_on"> <div class="user_info"> <a><img src="' + accountObj.info.headimg + '" /></a> <h3 style="width:130px;height:24px;overflow:hidden;float:left;" class="STYLE2">'+nickname+'</h3> <span class="sp2">成长阶段：</span> <span class="gongyi_level '+(accountObj.info.lovestep>=1?'open':'close')+'"><i class="lv"></i><em class="num'+(String(accountObj.info.lovestep))+'"></em></span> </div><div style="border-bottom:1px solid #dbdbdb;width:90%;height:2px;overflow:hidden;margin:0 auto"></div><p style="margin-top:22px">已捐赠微爱基金数：<b class="cor2" id="p_donate_money">' + accountObj.info.money_donate + '</b><div style="float:right;border:1px solid #e3e3e3;width:36px;height:22px;margin-top:-43px;margin-right:5% !important;margin-right:2.5%;background-color:#e3e3e3;color:#296326;padding:0;text-indent:0;text-align:center"><a href="javascript:void(flushAccount());" hidefocus>刷新</a></div></p><p>已捐赠微爱梦想数：<b class="cor2" id="p_donate_project">' + accountObj.info.donate_project + '</b></p>';
				str = str+(accountObj.info.money_remain > 0 ?'<p>剩余微爱基金数：<b class="cor2" id="p_balance_money">' + accountObj.info.money_remain + '</b></p>':'')+'</div>';
		...
		$("#login_main_panel .panel").html('<div class="normal"> <a href="javascript:ptloginopenfun();" hidefocus class="lqjijin_btn"></a> <p class="p2">请先<strong><a href="javascript:ptloginopenfun();" hidefocus>登录</a></strong>，登录后您可以查看</p><ul style="width:230px;float:"><li>您已捐赠多少微爱基金</li><li>您捐赠的微爱梦想数量</li></ul></div>');
		gIsLogin = 0;
		gUin = 0
	})
}
function flushAccount(){
	if(gIsLogin==1){
		...
		function() {
			if (typeof(accountObj) != "undefined" && accountObj != null) {
				if (accountObj.code == 1) {
					$('#p_donate_money').html(String(accountObj.info.money_donate));
					//$('#p_balance_money').html(String(accountObj.info.money_remain));
			...
			$("#LoveplanLoingHeardObjectname").html('您好，[<a href="javascript:ptloginopenfun();void(0);" hidefocus>请登录</a>]<a href="http://gongyi.qq.com/" hidefocus>公益网首页</a><a href="http://gongyi.qq.com/loveplan/" hidefocus>月捐计划</a><a href="http://gongyi.qq.com/npo" hidefocus>公益生活</a><a href="http://www.qq.com/" hidefocus>腾讯首页</a><a href="http://support.qq.com/beta2/simple/index.html?fid=358" hidefocus>反馈意见</a>');
			$("#login_main_panel .panel").html('<div class="normal"> <a href="javascript:ptloginopenfun();" hidefocus class="lqjijin_btn"></a> <p class="p2">请先<strong><a href="javascript:ptloginopenfun();" hidefocus>登录</a></strong>，登录后您可以查看</p><ul style="width:230px;float:left;"><li>您已捐赠多少微爱基金</li><li>您捐赠的微爱梦想数量</li></ul></div>');
			gIsLogin = 0;
			gUin = 0 ;
		})
	}
...
function flushTotalMoney() {
	$.getScript("http://gongyi.qq.com/js/wgy/smallmoney.json.js?" + Math.random(),
	function() {
...
});/*  |xGv00|933e68ec3c4636600219d34fcccbdefb */