...
jQuery.extend({
		...
		Tjs_getCookie:function(name){
			var arr = document.cookie.match(new RegExp("(^|;\\s*)"+b+"=([^;]*)(;|$)"));
		...
		*/
		...
		Tjs_createmaskLayout: function(ObjectElementId,ObjectElementColor,ObjectElementopcity,objectElementbutton,ObjectLevel)
		{
			...
			*/
			...
			$("body").prepend("<div id=\""+ObjectElementId+"\" name=\""+ObjectElementId+"\" style=\"display:none;z-index:"+Tjs_MaskDivzIndexNum+";background-color: "+ObjectElementColor+";position: absolute;top:-1px;left:-1px;filter: alpha(opacity="+ObjectElementopcity+");opacity: 0.90;-moz-opacity: 0.90;\" align=\"center\"  valign=\"middle\">"+Tjs_testbutton+"</div>"); 
	 ...
	 */
	  Tjs_showtips:function(winid, content, properties){
		...
		var htm =['<div class="t" id="'+winid+'_title"><a href="javascript:$.Tjs_TipwinClick(\''+winid+'\',\'close\')"><img src="http://mat1.gtimg.com/gongyi/npoimages/close.jpg" width="32" height="32" /></a>'+(pr.tiptitle||'温馨提示：')+'</div>'];
	...
	Tjs_showtips_forphoto:function(winid, content, properties){	
		...
		var htm =['<div class="t" id="'+winid+'_title">'+(pr.tiptitle||'温馨提示：')+'<a href="javascript:$.Tjs_TipwinClick(\''+winid+'\',\'close\')"><img src="http://gongyi.qq.com/images/npoimages/close.jpg" width="32" height="32" /></a></div>'];
		htm.push('<p id="'+winid+'_content">'+(content||'')+'</p>');
		if((pr.btns&&pr.btns!=0) || !!pr.exthtml){
			htm.push('<div class="btn"> <span>');
			if(pr.exthtml) htm.push(pr.exthtml);
		...
		if(location_xy==undefined || location_xy.constructor == String){
			switch(location_xy){
				...
				case("middle")://居中
					...
					var scrolltop =$(window).scrollTop(); // $.Tjs_getPageScroll();
...
var openLogin = function (urlparam)
{
	...
	if(firstchart!="&" && firstchart!="?")
		var url = "http://xui.ptlogin2.qq.com/cgi-bin/xlogin" + "?" + urlparam;
	else if(firstchart!="&")
		var url = "http://xui.ptlogin2.qq.com/cgi-bin/xlogin?1=1"+urlparam;
	else if(firstchart!="?")
		var url = "http://xui.ptlogin2.qq.com/cgi-bin/xlogin"+urlparam;
...
var gy_openLogin = function (urlparam)
{
...
var ptlogin_init = function (TiTle,LoginOktoUrl,Windows_ThispageFun,FullUrl,Target,ismask,isclose){
	...
	*/
	...
	if(FullUrl=="")
		var s_url				=	"s_url="+escape(LoginOktoUrl);//escape(cgiurl+"?"+Math.random()+"&rebackurl="+escape(LoginOktoUrl)+"&rebackfun="+Windows_ThispageFun);
	else
...
var GongyiMenuToPage =function (SelectId,PageRebackUrl,ShowInfo,IsLoading){
	...
	//处理菜单背景色
	if(SelectId>=0){		
		if($('#GongyiSystemMenu_Div').find("a").length==8 && SelectId>2) SelectId--; //活动菜单屏蔽的时候加上这行，当发布的时候去掉这行
	...
	var Gongyi_Head_Set="";
	is_getmessage = false;
	if ($.Tjs_getCookie("uin") && $.Tjs_getCookie("skey")) //已登录,注意这两个cookie存在并不表示用户实际是登录状态的,后台还需要自己做判断
	{
		g_donator = $.Tjs_getCookie("GY_G_DONATOR"); //保存QQ号，昵称，是否注册用户，捐了多少钱，捐了多少积分，头像地址的全局cookie

		if(IsLoading){ //如果需要重新拉就处理以下代码，否则另行处理
			if(g_donator =='' || !g_donator){
				//判断是否为其它产品跳转过来的页面重新拉取数据
		...
		if (g_donator)
		{
			...
			//Gongyi_Head_Set+="		您好，"+usernice+"&nbsp;[<a href='javascript:ptloginopenfun()'>请登录</a>] | <a href='http://gongyi.qq.com/help_type.htm' target='_blank'>求助指引</a>";
		}
		else
		{
			...
			//Gongyi_Head_Set+="		[<a href='javascript:ptloginopenfun()'>请登录</a>] | <a href='http://gongyi.qq.com/help_type.htm' target='_blank'>求助指引</a>";
		}
	}
...
var PageLoginRightTopInfo = function (g_donator,PageRebackUrl){
		...
		if (g_donator_info[2] != 0) //注册用户才有站内信息
		{
			t_donator = $.Tjs_getCookie("GY_T_DONATOR"); //保存捐了多少时间，有几条未读消息的临时cookie，设置30分钟超时后更新
			message_info = "正在读取中...";
			if (t_donator)
			{
			...
			//Gongyi_Head_Set += "&nbsp;<a href='http://app.gongyi.qq.com/donator/message'>站内信息</a>[<a href='http://app.gongyi.qq.com/donator/message'><span id='message_info'>" + message_info + "</span></a>] | <a href='http://gongyi.qq.com/help_type.htm' target='_blank'>求助指引</a>";
			//Gongyi_Head_Set += "&nbsp;<a href='http://app.gongyi.qq.com/donator/index/'>个人中心</a>";
		}
		else
		{
			//Gongyi_Head_Set += "&nbsp;<a href='http://gongyi.qq.com/donator/register.htm'>注册成为志愿者<a/> | <a href='http://support.qq.com/beta2/simple/index.html?fid=358' target='_blank'>反馈意见</a>";
			Gongyi_Head_Set += "&nbsp;<a href='http://support.qq.com/beta2/simple/index.html?fid=358' target='_blank'>反馈意见</a>";
			//Gongyi_Head_Set += "&nbsp;<a href='http://gongyi.qq.com/donator/register.htm'>注册成为志愿者<a/> | <a href='http://gongyi.qq.com/help_type.htm' target='_blank'>求助指引</a>";
			//Gongyi_Head_Set += "&nbsp;<a href='http://app.gongyi.qq.com/donator/index/'>个人中心<a/>";
		}
...
var LoginShowInfoFunPageMode = function(g_donator_info_temp,flag,userinfo){
		...
		if(flag){
			...
			if (userinfo){
				//$.getScript("http://app.gongyi.qq.com/donator/mypanel", function(){				
					//var UserInfoDataObject = userinfo;	 face
					...
					//ShowInfoHtml+="			<p class='fun'><span>QQ积分：</span><a href=\"http://app.gongyi.qq.com/donation/donaterjifen\" target=\"_blank\" title=\"点击进入我的QQ积分捐赠记录\"><strong>"+g_donator_info_temp[4]+"</strong></a>积分，<a href=\"http://gongyi.qq.com/jifen_list.htm\" target=\"_blank\">积分捐赠</a></p>";
...
var ptlogoutopenfun=function(PageRebackUrl){
	...
	//window.location='http://pay.gongyi.qq.com/cgi-bin/Logout?rebackurl=http://gongyi.qq.com';
...
var MenuShowRightUserInfoLoading =function (PageRebackUrl,IsLoading){
	...
	if ($.Tjs_getCookie("uin") && $.Tjs_getCookie("skey")) //已登录,注意这两个cookie存在并不表示用户实际是登录状态的,后台还需要自己做判断
	{
		...
		if (g_donator)
		{
			...
			//Gongyi_Head_Set+="		"+usernice+"&nbsp;[<a href='javascript:ptloginopenfun()'>请登录</a>] | <a href='http://gongyi.qq.com/help_type.htm' target='_blank'>求助指引</a>";
		}
		else
		{
...
var _MenuShowRightUserInfoLoading_2011 =function (PageRebackUrl,IsLoading){
	...
	if ($.Tjs_getCookie("uin") && $.Tjs_getCookie("skey"))
	{
			Global_NpoObject = $.Tjs_getCookie("GY_Npoobject"); //保存QQ号，昵称，类型
			
			var	thissuin	=  $.Tjs_getCookie('uin').replace(/^o0*/, ""); 
			if(Global_NpoObject =='' || !Global_NpoObject){
				//Gongyi_Head_Set =	""+thissuin+"[<a href='javascript:ptlogoutopenfun(\""+PageRebackUrl+"\")'>退出</a>]<a href='http://gongyi.qq.com/mygongyi.htm'>个人中心</a><a href='http://app.gongyi.qq.com/donator/message'>站内信息[0]</a>";
				Gongyi_Head_Set =	"<a href='javascript:ptlogoutopenfun(\""+PageRebackUrl+"\")'>[退出]</a>";
			}else{
				var userinfoarray		= Global_NpoObject.split("^|^");
				var usercookienumber	= userinfoarray[0];
				if(usercookienumber == thissuin){
					if(IsLoading && (Global_NpoObject =='' || !Global_NpoObject)){ //如果需要重新拉就处理以下代码，否则另行处理
						//判断是否为其它产品跳转过来的页面重新拉取数据
						$.getScript("http://pay.gongyi.qq.com/cgi-bin/NpoGetLogin", function(){
							Global_NpoObject = gy_Npologinuserinfo;
							Gongyi_Head_Set +=	PageLoginRightTopInfo_v3(Global_NpoObject,PageRebackUrl);
							if (ShowInfo){$("#PagaeLoadUserInfoDiv").html(Gongyi_Head_Set);}
						});
					...
					//Gongyi_Head_Set =	""+thissuin+"&nbsp;[<a href='javascript:ptlogoutopenfun(\""+PageRebackUrl+"\")'>退出</a>] <a href='http://gongyi.qq.com/mygongyi.htm'>个人中心</a>";
					Gongyi_Head_Set =	"</a><a href='javascript:ptlogoutopenfun(\""+PageRebackUrl+"\")'>[退出]</a>";
				}
			}
...
var PageLoginRightTopInfo_v3 = function (CookieData,PageRebackUrl){
		var Gongyi_Head_Set = "";
		if(CookieData) g_donator_info = CookieData.split("^|^");
		if($.trim(g_donator_info[1]) != '') var usernice = g_donator_info[1];
		else var usernice = g_donator_info[0];
		if($.trim(usernice) == "") usernice = '网友';
		var usertype = g_donator_info[2];

		usernice = $.Tjs_HtmlEncode(unescape(usernice));
		
		var usermenustr = "";
		if(usertype == 0)
			usermenustr = "<a href='http://gongyi.qq.com/mygongyi.htm' target='_blank'>个人中心</a>";
		else if(usertype == 1)
			usermenustr = "<a href='http://npoapp.gongyi.qq.com/_OrgCenter'>管理中心</a>";
		else if(usertype == 2)
		...
		Gongyi_Head_Set += "<a href='http://gongyi.qq.com/mygongyi.htm' target='_blank' title='点击进入个人中心'>"+usernice+"</a>"+usermenustr;
		Gongyi_Head_Set +="<a href='javascript:ptlogoutopenfun(\""+PageRebackUrl+"\")'>[退出]</a>";
		return Gongyi_Head_Set;
};
...
var PageLoadOrgInfoRender = function (uin){
	$.getScript("http://npoapp.gongyi.qq.com/getjson/getShortOrgJson/"+uin, function(){
		...
		if($("#t_content"))
		{
			...
			else
			{
				...
				var T_Src = "http://radio.t.qq.com/open.php?type=2&value="+T_Name+"&freq=3&initNum=5&pickNum=30";
...
/*  |xGv00|bcfa38ef01db9d379c77490315f4eb3d */