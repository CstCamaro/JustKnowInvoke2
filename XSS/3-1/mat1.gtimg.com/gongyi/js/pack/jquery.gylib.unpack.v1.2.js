...
(function(){
	...
	GyLib.Login=function(options){
		...
		function getRequestStr(){
			...
			*/
			...
			if(_opts.fullUrl=='')
				_opts.fullUrl = "s_url="+escape(_opts.returnUrl);//+escape(_opts.cgiurl+"?"+Math.random()+"&rebackurl="+escape(_opts.returnUrl));
			else
	...
	***************/
	...
	GyLib.Donate=function(options){
		...
		function _setFormObj(){
			...
			if(_formObj[0]==null){
				...
				if(!!_et && _et != ""){
					$('<input type="hidden" id="entry_type" name="entry_type" value="'+_et+'">').appendTo(_formObj);
				} 
			}
			_setDonateDiv();
		}
	...
	GyLib.Donate.__pc_wx_donate_url__ = 'http://gongyi.qq.com/succor/wxpay.v2.htm';
...
var _mtac = {};
 ...
 })();/*  |xGv00|2340a56cc3d4bc26b4dcf2856ef043bd */